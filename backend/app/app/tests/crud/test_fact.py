...
#     user = create_random_user(db)
#     fact = crud.fact.create_with_owner(db=db, obj_in=fact_in, owner_id=user.id)
#     stored_fact = crud.fact.get(db=db, id=fact.fact_id)
#     assert stored_fact
#     assert fact.fact_id == stored_fact.fact_id
...
#     user = create_random_user(db)
#     fact = crud.fact.create_with_owner(db=db, obj_in=fact_in, owner_id=user.id)
#     fact2 = crud.fact.remove(db=db, id=fact.fact_id)
#     fact3 = crud.fact.get(db=db, id=fact.fact_id)
#     assert fact3 is None
#     assert fact2.id == fact.fact_id
