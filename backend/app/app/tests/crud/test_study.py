...
def test_get_eligible_facts(db: Session) -> None:
    ...
    study_facts = crud.fact.get_eligible_facts(db=db, user=user, deck_ids=[default_deck.id])
    assert len(study_facts) == 1
    assert study_facts[0] == user1_facts[0]
    assert study_facts[0].user_id == user.id

    study_facts_no_deck = crud.fact.get_eligible_facts(db=db, user=user)
    assert len(study_facts_no_deck) == 1
    assert study_facts_no_deck[0] == user1_facts[0]
    assert study_facts_no_deck[0].user_id == user.id

    study_facts_2 = crud.fact.get_eligible_facts(db=db, user=user2, deck_ids=[default_deck.id])
    assert len(study_facts_2) == 1
    assert study_facts_2[0] == user2_facts[0]
    assert study_facts_2[0].user_id == user2.id

    study_facts_2_no_deck = crud.fact.get_eligible_facts(db=db, user=user2)
    assert len(study_facts_2_no_deck) == 2
    user1_facts.append(create_random_fact_with_deck(db, user=user, deck=private_deck))
    study_facts_with_public_fact = crud.fact.get_eligible_facts(db=db, user=user)
    assert len(study_facts_with_public_fact) == 2
    study_facts_with_public_fact_user_2 = crud.fact.get_eligible_facts(db=db, user=user2)
