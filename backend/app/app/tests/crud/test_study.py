...
def test_get_eligible_facts(db: Session) -> None:
    ...
    study_facts = crud.fact.get_eligible_facts(db=db, user=user, filters=FactSearch(deck_id=default_deck.id, all_suspended=True))
    assert len(study_facts) == 1
    assert study_facts[0] == user1_facts[0]
    assert study_facts[0].user_id == user.id

    study_facts_no_deck = crud.fact.get_eligible_facts(db=db, user=user, filters=FactSearch(all_suspended=True))
    assert len(study_facts_no_deck) == 1
    assert study_facts_no_deck[0] == user1_facts[0]
    assert study_facts_no_deck[0].user_id == user.id

    study_facts_2 = crud.fact.get_eligible_facts(db=db, user=user2, filters=FactSearch(deck_id=default_deck.id, all_suspended=True))
    assert len(study_facts_2) == 1
    assert study_facts_2[0] == user2_facts[0]
    assert study_facts_2[0].user_id == user2.id

    study_facts_2_no_deck = crud.fact.get_eligible_facts(db=db, user=user2, filters=FactSearch(all_suspended=True))
    ...
    study_facts_with_public_fact = crud.fact.get_eligible_facts(db=db, user=user, filters=FactSearch(all_suspended=True))
    assert len(study_facts_with_public_fact) == 2
    study_facts_with_public_fact_user_2 = crud.fact.get_eligible_facts(db=db, user=user2, filters=FactSearch(all_suspended=True))
...
def test_get_eligible_facts_two_owners(db: Session) -> None:
    ...
    user1_study = crud.fact.get_eligible_facts(db=db, user=user1, filters=FactSearch(all_suspended=True))
    user2_study = crud.fact.get_eligible_facts(db=db, user=user2, filters=FactSearch(all_suspended=True))
    user3_study = crud.fact.get_eligible_facts(db=db, user=user3, filters=FactSearch(all_suspended=True))
