  ...
  <v-container fluid style="max-width:1250px">
    ...
    <v-card class="mx-3 my-1 py-1 px-0 px-sm-3">
      <v-card-title primary-title class="mx-3 my-0 pa-0">
        ...
        <div v-else-if="$vuetify.breakpoint.xsOnly || studyset === null" class="headline primary--text">
          Learn
        </div>
        <div v-else class="headline primary--text">
          {{ studyset.short_description }}
        </div>
        <div v-show="show.text !== `Loading...`" class="headline primary--text" style="margin-left: .5em">
        ...
        <span v-show="!inTestMode">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-if="$vuetify.breakpoint.mdAndDown" :disabled="!show.enable_actions" text icon v-bind="attrs"
                @click="dialog = true" v-on="on">
                <v-icon>mdi-information</v-icon>
              </v-btn>
              <v-btn v-else :disabled="!show.enable_actions" class="ma-1 pa-2" v-bind="attrs" @click="dialog = true"
                v-on="on">
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              ...
              <v-btn v-else :disabled="!show.enable_actions" class="ma-1 pa-2" v-bind="attrs" @click="mark()" v-on="on">
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-if="$vuetify.breakpoint.mdAndDown" :disabled="!show.enable_actions" text icon v-bind="attrs"
                @click="suspend()" v-on="on">
                <v-icon>mdi-pause</v-icon>
              </v-btn>
              <v-btn v-else :disabled="!show.enable_actions" class="ma-1 pa-2" v-bind="attrs" @click="suspend()"
                v-on="on">
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-if="$vuetify.breakpoint.mdAndDown" :disabled="!show.enable_actions" text icon v-bind="attrs"
                @click="remove()" v-on="on">
                <v-icon>mdi-delete</v-icon>
              </v-btn>
              <v-btn v-else class="ma-1 pa-2" :disabled="!show.enable_actions" v-bind="attrs" @click="remove()" v-on="on">
          ...
          <span v-if="show.enable_report">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn v-if="$vuetify.breakpoint.mdAndDown" text icon v-bind="attrs" @click="report()" v-on="on">
                  <v-icon>mdi-alert-octagon</v-icon>
                </v-btn>
                <v-btn v-else class="ma-1 pa-2" v-bind="attrs" @click="report()" v-on="on">
          ...
          <span v-else-if="show.enable_actions">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn v-if="$vuetify.breakpoint.mdAndDown" text icon v-bind="attrs" @click="edit()" v-on="on">
        ...
        <v-row no-gutters>
          ...
          <v-col cols="12" sm="auto">
            <div v-show="show.enable_show_back && show.fact && show.fact.identifier" class="title">
              ...
              <span v-if="show.fact &&
                show.fact.deck &&
                show.fact.deck.title !== 'Vocab'" class="hidden-xs-only">Identify {{ show.fact.identifier }}</span>
      ...
      <v-card-text v-show="show.enable_show_back" class="py-2">
        <v-text-field id="answer" ref="answerfield" v-model="typed" solo autofocus hide-details="auto"
          @keydown="keyHandler" v-if="show.enable_show_back &&
            show.fact &&
            show.fact.category &&
            show.fact.category.toString() !== 'GRE'"><template v-slot:label>
    ...
    <v-card v-show="showBack && show.enable_show_back" class="my-2 mx-3 px-3 py-4">
      ...
      <v-card-text class="pb-0 pt-1">
        <div v-if="show.enable_show_back &&
          show.fact &&
          show.fact.deck &&
          show.fact.deck.title !== 'Vocab'" class="title primary--text">
      ...
      <v-card-text v-show="show.enable_show_back" class="py-2">
        <v-text-field id="retype_answer" ref="retype" v-model="retyped" solo
          label="Optional - Retype Answer (Press any letter to focus)" autofocus hide-details="auto" v-if="show.enable_show_back &&
            show.fact &&
            show.fact.category &&
            show.fact.category.toString() !== 'GRE'"></v-text-field>
      </v-card-text>
      <v-card-actions class="pt-3 pb-1 px-5">
        <v-row class="shrink" justify="space-around">
          <v-col v-show="showResponseBtns" cols="5" sm="auto" class="ma-1 pa-1 py-0 shrink">
            <v-btn ref="wrong" :color="!recommendation ? 'red' : ''" class="px-2" @click="response(false)">wrong
              ([)</v-btn>
          </v-col>
          <v-col v-show="showResponseBtns" id="response" cols="5" sm="auto" class="ma-1 pa-1 py-0 shrink">
            <v-btn ref="right"
              :color="recommendation || (show.fact && show.fact.deck && show.fact.deck.title === 'Vocab') ? 'green' : ''"
              class="px-2" @click="response(true)">right
              (])</v-btn>
          </v-col>
          <v-col v-show="!showResponseBtns" cols="5" sm="auto" class="ma-1 pa-1 py-0 shrink">
            <v-btn ref="continue" class="px-2" @click="response(false)">continue (Enter)</v-btn>
          </v-col>
        </v-row>
      </v-card-actions>
    </v-card>
    <v-card
      v-show="showBack && show.enable_show_back && show.fact && show.fact.category && show.fact.category.toString() === 'GRE'"
      class="my-2 mx-3 px-3 py-4" color='#e0f0ff'>
      <v-card-title class="py-0">
        <v-row>
          <v-col cols="12" rows="2">
            <b>KAR³L-generated Mnemonic Device</b>
          </v-col>
        </v-row>
      </v-card-title>
      <v-card-text v-show="showBack && show.fact && show.fact.extra && show.fact.extra['mnemonic_1']" class="pb-0 pt-1">
        <div class="title primary--text">
          {{ show.fact.extra['mnemonic_1'] }}
        </div>
      </v-card-text>
      <v-card-text>
       
      <v-row>
        <v-col cols="12"><v-radio-group
          style="color:black" label="Submit Feedback (Optional)" v-model="radio" required :rules="radioRules">
            <v-radio label="Good Mnemonic" value="a"></v-radio>
            <v-radio label="Incorrect Definition" value="b"></v-radio>
            <v-radio label="Difficult to Understand" value="c"></v-radio>
            <v-radio label="Bad Keyword Link" value="d"></v-radio>
            <v-radio label="Offensive" value="e"></v-radio>
          </v-radio-group><v-btn>Submit</v-btn></v-col>
      </v-row>
    </v-card-text>
    </v-card>
    ...
    <v-dialog v-model="editDialog" max-width="1000px" scrollable @click:outside="returnLearn">
...
export default class Learn extends Vue {
  ...
  random_fact = '';
  mdiHeadCogOutline = mdiHeadCogOutline;
